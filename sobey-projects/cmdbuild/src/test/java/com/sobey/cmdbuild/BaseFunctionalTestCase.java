...
public class BaseFunctionalTestCase {
	...
	protected static void startJettyOnce() throws Exception {
		if (jettyServer == null) {
			...
			jettyServer = JettyFactory.createServerInSource(new URL(baseUrl).getPort(), CMDBuildServer.CONTEXT);
			JettyFactory.setTldJarNames(jettyServer, CMDBuildServer.TLD_JAR_NAMES);
			jettyServer.start();
