...
public class ServerPortSoapTest extends BaseFunctionalTestCase {
	...
	public void find() {
		...
		DTOResult<ServerPortDTO> dtoResult = cmdbuildSoapService.findServerPortByParams(searchParams);
	...
	public void save() {
		...
		ServerPort port = TestData.randomServerPort();
		ServerPortDTO dto = BeanMapper.map(port, ServerPortDTO.class);
		IdResult response = cmdbuildSoapService.createServerPort(dto);
	...
	public void update() {
		...
		ServerPortDTO dto = response.getDto();
		dto.setDescription(dto.getDescription() + "Update");
		IdResult result = cmdbuildSoapService.updateServerPort(id, dto);
